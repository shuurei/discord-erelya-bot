...
export default new Event({
    ...
    async run({ events: [message] }) {
        ...
        const database = {
            ...
            member: await MemberService.findOrCreate({ userId, guildId })
        }
        ...
        if (usedPrefix) {
            ...
            if (command.messageCommand.style === MessageCommandStyle.SLASH_COMMAND) {
                args.splice(0, command.structure.message!.parts!.length + 1);
            } else {
                args.splice(0, 1);
            }

            return this.client.emit('commandCreate', command, message, database, args);
        }
