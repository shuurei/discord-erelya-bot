...
import { applicationEmojiHelperSync, parseChannelMention } from '@/utils'
...
export default new Command({
    ...
    async onMessage(message, { args: [channelId] }) {
        ...
        const { whiteArrowEmoji } = applicationEmojiHelperSync();

        let targetChannelId: string | null = null;
        let targetLabel = 'Guilde entière';

        if (channelId) {
            targetChannelId = parseChannelMention(channelId) ?? channelId;
